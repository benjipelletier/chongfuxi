   ...
   <div 
    ...
    @click="clickHandler"
     > 
        ...
        <div v-if="getUserProgress.readyHas(character)" class=" bg-white bg-opacity-20 w-2 h-2 rounded-full absolute top-1 right-1 text-gray-200">
        </div>
    </div> 
</template>
...
export default {
    ...
    computed: {
        ...
        cardBgStyle() { 
            return StyleCalc.cardBgStyle(this.sectionId, this.reviewLevel, this.character);
        },
        cardStyle() { 
            return StyleCalc.cardStyle(this.sectionId, this.reviewLevel, this.character);
        },
        cardSizeStyle() {
            return {
